...
class User
{
    ...
    {
        $username = $this->user['user_name'];
        $query = mysqli_query($this->con, "SELECT profile_pic FROM user WHERE user_name='$username'");
        $row = mysqli_fetch_array($query);
    ...
    {
        $username = $this->user['user_name'];
        $query = mysqli_query($this->con, "SELECT friend_array FROM user WHERE user_name='$username'");
        $row = mysqli_fetch_array($query);
    ...
    public function removeFriend($user_to_remove)
    {
        ...
        $querry = mysqli_query($this->con, "SELECT friend_array FROM user WHERE user_name='$user_to_remove'");
        $row = mysqli_fetch_array($querry);
        $friend_array_username = $row['friend_array'];

        $new_friend_array = str_replace($user_to_remove . ",", "", $this->user['friend_array']);
        $remove_friend = mysqli_query($this->con, "UPDATE user SET friend_array='$new_friend_array' WHERE user_name='$logged_in_user'");

        $new_friend_array = str_replace($this->user['user_name'] . ",", "", $friend_array_username);
        $remove_friend = mysqli_query($this->con, "UPDATE user SET friend_array='$new_friend_array' WHERE user_name='$user_to_remove'");
    }
    ...
    public function getMutualFriends($user_to_check)
    {
        ...
        $query = mysqli_query($this->con, "SELECT friend_array FROM user WHERE user_name='$user_to_check'");
        $row = mysqli_fetch_array($query);
